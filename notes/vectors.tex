...
\begin{ex}
	...
	The general form of these solutions is $(Ax+B)e^{mx}$.

	We know from our particular solution that $y(0) = 1$ and $y'(0)
	= 1$, hence $B = 1$ and

	\begin{gather*}
		\left(-3(Ax+B)e^{-3x} + Ae^{-3x}\right)(0) = 1 \\
		\Rightarrow -3B + A = 1 \\
		\Rightarrow A = 4
	\end{gather*}
...
\begin{ex}
	...
	\begin{align*}
		...
		\sin(-\theta) &= -\sin(\theta)
	\end{align*}
	our particular solution can be simplified to
	\[
		y = e^{-x/2}\left(\cos \frac{\sqrt{3}}{2}x
		\pm i\sin\frac{\sqrt{3}}{2}x\right)
	\]

	The general solution $y = (Ax+B)e^{mx}$:
	\begin{align*}
		y(0) &= 1 \Rightarrow B = 1 \\
		y'(0) &= \left( (Ax+B)me^{mx} + Ae^{mx} \right)(0) = 1 \\
		\Rightarrow A &= 1 - m(0) \\
		&= 1 - e^{0}\left(\cos(0) \pm i\sin(0)\right) \\
		&= 1 - 1 \\
		&= 0 \\
		\Rightarrow y &= e^{-x/2}\left(\cos \frac{\sqrt{3}}{2}x
		\pm i\sin \frac{\sqrt{3}}{2}x\right)
	\end{align*}
\end{ex}
...
\begin{ex}
	...
	\begin{gather*}
		y_c = (Ax+B)e^{2x} \\
		B = 1 \\
		y_c' = 2(Ax+B)e^{2x} + Ae^{2x} \\
		\Rightarrow 2B + A = 1 \Rightarrow A = -1 \\
		\Rightarrow y_c = (1-x)e^{2x}
	\end{gather*}
	Now we can calculate the general solution $y = y_c + y_p$:
	\[
		y = (1-x)e^{2x} + \frac{1}{4}x + \frac{1}{4}
	\]
\end{ex}
